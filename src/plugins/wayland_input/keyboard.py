...
def single_keypress(key: str = "", presses: int = 1):
    ...
    Keywords: keyboard, key_press, press, wayland, Linux-MCP
    """
    keyboard = Keyboard()
    for _ in range(presses):
        keyboard.press(_fix_pyautogui_keys([key])[0])
        sleep(0.05)
...
def hotkey_press(hotkey:list[str]):
    ...
    Supports special keys like "enter", "escape", "tab", "space", "backspace", "delete",
    arrow keys ("up", "down", "left", "right"), function keys ("f1"-"f12").
    Keywords: keyboard, key_press, press, hotkey, wayland, Linux-MCP
    """
    keyboard = Keyboard()
    keyboard.hotkey(*_fix_pyautogui_keys(hotkey))
...
def type_line_of_text(text: str) -> str:
    ...
    Keywords: keyboard, key_press, press, text, type, text_type, wayland, Linux-MCP
    """
    warning_chars = []
    keyboard = Keyboard()
    for char in text:
